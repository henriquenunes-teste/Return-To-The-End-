...
import com.code.dao.UpgradeDAO;
...
public class SelectionController implements Initializable {
    ...
    public void go(Integer id) throws IOException, SQLException{
        ...
        PlayerDAO dao = new PlayerDAO();
        Upgrade upgrade = dao.getUpgrade(Session.session.getPlayer());
        
        if(upgrade == null){
            UpgradeDAO upDao = new UpgradeDAO();
            upDao.create(Session.session.getPlayer());
            upgrade = dao.getUpgrade(Session.session.getPlayer());
            
            Session.session.getPlayer().setUpgrade(upgrade);
        }else{
            Session.session.getPlayer().setUpgrade(upgrade);
        }
